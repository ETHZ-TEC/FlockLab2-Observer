...
do
    ...
    RES=$(rsync -a -z -c -i --delete --timeout=5 --dry-run --exclude=".git" -e "ssh -q -p ${PORT}" ../observer/ ${USER}@${HOSTPREFIX}${OBS}:observer/  2>&1)
    if [ $? -ne 0 ]; then
    ...
    rsync -a -q -z -c --delete --exclude=".git" -e "ssh -q -p ${PORT}" ../observer/ ${USER}@${HOSTPREFIX}${OBS}:observer/
    if [ $? -ne 0 ]; then
