...
class Stm32Bootloader:
    ...
    def extended_erase_memory(self):
        ...
        debug(5, "Extended erase (0x44), this can take ten seconds or more")
    ...
    def perform_commands(self):
        ...
        if self.configuration['verify']:
            ...
            if binary_data == read_data:
                debug(5, "Verification OK")
            else:
                debug(0, "Verification FAILED")
...
"""
    ...
    def read_device_details(self):
        ...
        debug(5, "Bootloader version %X" % boot_version)
        device_id = self.bootloader.get_id()
        debug(5, "Chip id: 0x%x (%s)" % (device_id, CHIP_IDS.get(device_id, "Unknown")))
        family = self.configuration['family']
        if not family:
            debug(5, "Supply -f [family] to see flash size and device UID, e.g: -f F1")
        else:
            device_uid = self.bootloader.get_uid(family)
            device_uid_string = self.bootloader.format_uid(device_uid)
            debug(5, "Device UID: %s" % device_uid_string)

            flash_size = self.bootloader.get_flash_size(family)
            debug(5, "Flash size: %d KiB" % flash_size)
        return device_id
