...
##############################################################################
def usage():
	print "Usage: %s --state={on,off} [--target=<int>] [--state] [--quiet] [--help] [--version]" %sys.argv[0]
	print "Turn target USB power on or off."
	print "Options:"
	print "  --state\t\t\tSet to on if target(s) USB should be turned on, set to off to turn off."
	print "  --target\t\t\tOptional. If set, the requested targets USB power is turned on. Otherwise all targets are turned on."
	print "  --status\t\t\tOptional. Used in combination with --target, returns 1 if target USB is on, 0 if target USB is off."
...
def main(argv):
	...
	# Open the syslog:
	openlog('tg_usbpwr', LOG_CONS | LOG_PID | LOG_PERROR, LOG_USER)
	...
	if status:
		rs = flocklab.tg_usbpwr_get(target)
		if not quiet:
			sys.stdout.write('%d\n'%rs)
		sys.exit(rs)
	...
	if (state == "on"):
		...
		value = 0
	if target:
		flocklab.tg_usbpwr_set(target, value)
	else:
		for i in range(1,5):
			flocklab.tg_usbpwr_set(i, value)
