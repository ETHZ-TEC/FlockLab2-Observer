...
class SerialForwarder():
	...
	def open(self):
		try:
			self.ser.open()
			syslog(LOG_INFO, "SerialForwarder started for device %s with baudrate %d"%(self.ser.port, self.ser.baudrate))
		except(Exception) as err:
			syslog(LOG_ERR, "SerialForwarder could not start because: %s" % (str(sys.exc_info()[1])))
			return None
		syslog(LOG_INFO, "SerialForwarder opened.")

	def close(self):
		self.ser.close()
		syslog(LOG_INFO, "SerialForwarder stopped")
	...
	def read(self):
		...
		try:
			...
			if (err[0] == 4):
				syslog(LOG_INFO, "SerialForwarder interrupted due to caught stop signal.")
		except:
			syslog(LOG_ERR, "SerialForwarder encountered error in line %d: %s: %s" % (traceback.tb_lineno(sys.exc_info()[2]), str(sys.exc_info()[0]), str(sys.exc_info()[1])))
		return ret
	...
	def write(self, data):
		try:
			...
			if rs != len(data):
				syslog(LOG_ERR, "SerialForwarder error while writing: no of bytes written (%d) != no of bytes in data (%d)." %str(rs, len(data)))
			self.num_elements_snd = self.num_elements_snd + 1
		except(socket.error) as err:
			syslog(LOG_ERR, "SerialForwarder error while writing to serial forwarder: %s" %str(err))
			self.close()
		except:
			syslog(LOG_ERR, "SerialForwarder encountered error in line %d: %s: %s" % (traceback.tb_lineno(sys.exc_info()[2]), str(sys.exc_info()[0]), str(sys.exc_info()[1])))
		return None
				...
				if data != None:
					...
					if len(data) > 0:
						try:
							...
							syslog(LOG_INFO, "[0,%s,%s]" %(str(data), str(timestamp)))
						except Queue.Full:
		...
		while stopLock.acquire(False):
			...
			try:
				...
				if _len > 0:
					...
					syslog(LOG_INFO, "SERVICE: %s - DATA: %s" % (str(_service), str(_data)))
...
##############################################################################
def usage():
	print("Usage: %s --testid=<int> [--port=<string>] [--baudrate=<int>] [--socketport=<int>] [--stop] [--daemon] [--debug] [--help] [--version]" %sys.argv[0])
	print("Options:")
...
def main(argv):
	...
	try:
		opts, args = getopt.getopt(argv, "ehvqdt:p:m:b:i:l:", ["stop", "help", "version", "daemon", "debug", "port=", "baudrate=", "testid=", "socketport="])
	...
	try:
		...
		print("Error when starting DB buffer process in line %d: %s: %s" % (traceback.tb_lineno(sys.exc_info()[2]), str(sys.exc_info()[0]), str(sys.exc_info()[1])))
		stop_on_sig(SUCCESS)
